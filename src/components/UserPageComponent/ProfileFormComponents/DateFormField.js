...
import { DateInput, FormField, styleData } from './ProfilePage.styled';
...
export default function DateFormField({ label, disabled, value, onChange }) {
  const convertDateToString = value => {
    ...
    const month = `${valueToDate.toDate().getMonth() + 1 < 10 && '0'}${
      valueToDate.toDate().getMonth() + 1
    }`;
    const date = `${valueToDate.toDate().getDate() < 10 ? '0' : ''}${valueToDate
      .toDate()
      .getDate()}`;
